...
/** the printable CV, rendered from whichever language the visitor picked */
export default function CvDocument() {
  const { t, content } = useLanguage();
  const { profile, roles, skills, education, languages, projects, engineering } = content;

  return (
    <main className="mx-auto max-w-3xl px-6 py-14 print:py-0">
      <div className="flex items-center justify-between print:hidden">
        <Link href="/" className="font-mono text-xs text-teal hover:underline">
          {t.cvBack}
        </Link>
        <LanguageToggle />
      </div>
      ...
      <section className="mt-8">
        <h2 className="font-mono text-xs uppercase tracking-widest text-teal">{t.cvSkills}</h2>
      ...
      <section className="mt-10">
        <h2 className="font-mono text-xs uppercase tracking-widest text-teal">{t.cvExperience}</h2>
      ...
      <section className="mt-10">
        <h2 className="font-mono text-xs uppercase tracking-widest text-teal">{t.cvProjects}</h2>
      ...
      <section className="mt-10">
        <h2 className="font-mono text-xs uppercase tracking-widest text-teal">{t.cvEngineering}</h2>
      ...
      <section className="mt-10">
        <h2 className="font-mono text-xs uppercase tracking-widest text-teal">{t.cvEducation}</h2>
      ...
      <section className="mt-10">
        <h2 className="font-mono text-xs uppercase tracking-widest text-teal">{t.cvLanguages}</h2>
        <ul className="mt-3 space-y-1 text-sm text-ink/80">
          {languages.map((l) => (
            <li key={l.name}>
